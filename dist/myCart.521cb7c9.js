...
})({"9p7Iz":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "ec77606b521cb7c9";
...
},{}],"4V6o6":[function(require,module,exports) {
...
function loadCart(cart1) {
    ...
    <h1>${_index.currencyFormat(total)}</h1>
    
  `;
    cartPriceTotal.appendChild(cartTotal);
    console.log(_index.currencyFormat(total));
}
async function removeProducts(productId) {
    const newCart = cart.filter((product)=>product.id !== productId
    );
    cart = newCart;
    if (userLogged) await _cart.createFirebaseCart(_importFirebase.db, userLogged.uid, newCart);
    _productDetail.addProductToCart(newCart);
...
function renderProducts(product) {
  ...
  <div>
      <p class="product__price">${_index.currencyFormat(product.price)}</p>
  </div>
  <div>
  ...
  `;
    ...
    productCart.addEventListener("click", (e)=>{
        e.preventDefault();
        if (e.target.tagName === "IMG") removeProducts(product.id);
    });
}
//----------Checkout------------
const deleteCart = async ()=>{
    try {
        await _firestore.deleteDoc(_firestore.doc(_importFirebase.db, "cart", userLogged.uid));
        loadCart([]);
...
const createOrder = async (userData)=>{
    try {
        const order = await _firestore.addDoc(_firestore.collection(_importFirebase.db, "orders"), {
            ...userData,
            products: cart,
            total
        });
...
btnCheckout.addEventListener("click", (e)=>{
    ...
    if (cart.length) {
        if (name && id && adress) createOrder(userData);
...
_auth.onAuthStateChanged(_importFirebase.auth, async (user)=>{
    ...
    } else cart = _productDetail.getMyCart();
    loadCart(cart);
});

},{"./importFirebase":"dYV9L","firebase/auth":"drt1f","./cart":"7K3yW","./productDetail":"cDZG2","./utils/index":"1FpIw","firebase/firestore":"cJafS"}]},["9p7Iz","4V6o6"], "4V6o6", "parcelRequireb12e")

//# sourceMappingURL=myCart.521cb7c9.js.map
